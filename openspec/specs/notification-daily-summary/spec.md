...
- **下段（其餘進行中）**：該看板**符合 ±7 補完窗口**（沿用既有 `_in_summary` 納入/逾期規則：未完成且執行當日 ≥ 補完後 `start`，執行當日 > 補完後 `end` 標逾期）但**未出現在上段**的工項，去重後列出、標逾期、無按鈕。
  ...
  - **呈現**：下段每筆進行中工項 SHALL 以與上段急迫項**相同的區塊格式**呈現——彩色粗體抬頭（`進行中`／逾期者 `已逾期`）＋灰色 `清單/卡片` 路徑＋深色**原始 tag 文字**（含 `[@(alias),期間]`）——不使用欄位分組小圓點樣式，且 MUST NOT 另加「其餘進行中工項」區段標題（避免與抬頭「進行中」重複）。card 層級工項（desc tag 未填 label）SHALL 以卡片名（路徑）＋其原始 tag 呈現，MUST NOT 因「label 等於卡片名且未逾期」而被略過。
...
- **WHEN** 呈現下段進行中工項
- **THEN** 每筆以彩色抬頭（`進行中`／`已逾期`）＋灰色 `清單/卡片` 路徑＋深色原始 tag 文字的區塊呈現，與上段「?天內到期」同款，不使用欄位分組小圓點，且不另加區段標題

#### Scenario: card 層級顯示原始 tag 資料
- **WHEN** 下段有 card 層級工項（desc tag，如 `70. [木工] 封板` `[@(木欽),20260626-20260715]`）
- **THEN** 區塊第三行顯示其原始 tag `[@(木欽),20260626-20260715]`，MUST NOT 遺漏 tag 資料
