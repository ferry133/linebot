## Purpose
定義 LINE 對話內的「線上說明」：使用者由 Rich Menu 入口進入，bot 依其 `line_users.role` 在聊天室內呈現對應角色手冊（`docs/<role>-guide.md`）的線上說明——主題選單、單一主題（含目前位置與前後/目錄/全部導覽）、完整手冊（標示所在位置）。全程走 Reply API，不開網頁、不耗推播額度。
