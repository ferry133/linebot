## Purpose
Defines how outbound LINE messages are delivered: immediate replies use the free Reply API (with the inbound reply token threaded through the pipeline), falling back to the Push API when no valid token exists; proactive messages always use Push.
