...
> 第 1～8 項皆以「含有 `[@...]` 標記的項目」為前提。第 9 項（每日摘要）固定發送，內容只包含**未完成**且「今天 ≥ 補完後的 start」的標記工項（半開區間以 ±7 補完：只有 end→`[end-7,end]`、只有 start→`[start,start+7]`）；今天 > 補完後的 end 者標示「逾期」紅字。未來才開始（今天 < 補完 start）、已完成者皆不列入。

| # | 檢查時間 | 條件 | 通知對象 | 訊息內容 |
|---|---------|------|---------|---------|
| 1 | Sun~Sat 12:00 | 今天距離**開始日**剩 1～7 天（每日） | sponsor | 「[工項名稱] X 天後開始，請準備」 |
| 2 | Mon~Sat 09:00 | 今天 = 開始日 | sponsor | 「[工項名稱] 今日開始，請確認」 |
| 3 | Sun~Sat 12:00 | 今天距離**結束日**剩 1～7 天（每日）且**未完成** | sponsor + 所有管理者/員工 | 「[工項名稱] X 天後到期」 |
| 4 | Mon~Sat 09:00 | 今天 = 結束日（時間未到）且**未完成** | sponsor + 所有管理者/員工 | 「[工項名稱] 今日（HH:MM）到期，請確認」 |
| 5 | Mon~Sat 18:00 | 今天 = 結束日（時間已過，需有 `:HHMM`）且**未完成** | sponsor + 所有管理者/員工 | 「[工項名稱] 今日 HH:MM 已逾期，請確認」 |
| 6 | Mon~Fri 18:00 | 結束日已過期且**未完成** | sponsor + 所有管理者/員工 | 「[工項名稱] 已逾期 X 天，請確認」 |
| 7 | Mon~Sat 12:00 | Checklist 停滯超過 3 天（該 checklist 需有 `[@...]` 標記） | 所有管理者/員工 | 「[卡片名稱] 已停滯 X 天，請追蹤」 |
| 8 | Mon~Sat 12:00 | Checklist 所有項目全部勾選（該 checklist 需有 `[@...]` 標記） | sponsor | 「[卡片名稱] 所有工項已全部完成 ✓」 |
| 9 | Mon~Sat 09:00 | 每日固定摘要（無條件發送） | 所有管理者/員工 | 未完成且「今天 ≥ 補完 start（±7 補半開區間）」的標記工項總覽，今天 > 補完 end 者標紅「逾期」；若無則顯示「今日無進行中工項」 |

> **通知對象說明**
> - **sponsor**：工項 `[@(...)]` 標記指定的對象（可多人），由 `line_users.alias_name` 解析。
> - **所有管理者/員工**：`line_users` 中 `role IN ('admin','employee')` 的全體帳號（內部提醒收件人；不再綁定特定 sa/larry）。DB 查無時該次內部份不發送，僅記 log。
