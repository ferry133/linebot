...
> 第 1～8 項皆以「含有 `[@...]` 標記的項目」為前提。第 9 項（每日摘要）固定發送，內容只包含**未完成**且「今天 > start（已開始）或 今天 > end（逾期）」的標記工項；今天 > end 者標示「逾期」紅字。未來才開始、只有 `end` 但未到期、已完成者皆不列入。
...
| 9 | Mon~Sat 09:00 | 每日固定摘要（無條件發送） | 所有管理者/員工 | 未完成且「今天 > start 或 今天 > end」的標記工項總覽（逾期者標紅）；若無則顯示「今日無進行中工項」 |
