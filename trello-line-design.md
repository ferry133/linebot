...
> 第 1～8 項皆以「含有 `[@...]` 標記的項目」為前提。第 9 項（每日摘要）固定發送，內容只包含**未完成**且「今天落在 `[start, end]` 區間內 或 已逾期（有 `end` 即可，含只有 `end`）」的標記工項；未來才開始、只有 `end` 但未到期、只有 `start` 無 `end`、已完成者皆不列入。
...
| 9 | Mon~Sat 09:00 | 每日固定摘要（無條件發送） | 所有管理者/員工 | 未完成且「今天在區間內 或 逾期」的標記工項總覽；若無則顯示「今日無進行中工項」 |
